...
## Tests (Emergency Vehicle Priority)

Because the vehicle is unavailable for this sprint, validation will focus on software-only evidence for the emergency path.

**Sprint:** Sprint 17 — 29/06/26 to 10/07/26

### Test scope

- Unit tests for the priority policy and scenario resolution.
- Tests for the traffic-light simulator emergency override.
- Tests for the emergency client request flow.
- Local demo / smoke test without a physical car.

### Ready-to-paste test list

**TEST-EP-01 — Unit test: emergency priority policy**
- Purpose: verify that emergency mode overrides normal flow and sets `priority_active=True`.
- Expected result: emergency request wins over normal request.
- Type: pure unit test.

**TEST-EP-02 — Integration test: traffic-light emergency simulator**
- Purpose: verify `emergency_on` switches the light to `emergency_green` and `emergency_off` restores normal behavior.
- Expected result: traffic-light state transitions match the emergency override rules.
- Type: integration test (simulator + coordinator flow).

**TEST-EP-03 — Unit/integration smoke test: emergency client request flow**
- Purpose: verify the emergency client publishes the priority state and requests the emergency-light action path.
- Expected result: `priority_active` becomes `True` during emergency mode and returns to `False` when cleared.
- Type: unit test with fake/mock dependencies for the traffic-light service and KUKSA publisher.

**TEST-EP-04 — Local smoke test: unified emergency demo**
- Purpose: run the local demo without a car and confirm the end-to-end emergency priority workflow.
- Expected result: emergency mode produces `emergency_green`, opens the barrier, and activates the emergency actions.
- Type: local smoke/integration demo.

### Existing test files that support this scope

- [src/mobility_scenarios_src/emergency_priority/tests/test_coordinator.py](src/mobility_scenarios_src/emergency_priority/tests/test_coordinator.py)
- [src/mobility_scenarios_src/emergency_priority/tests/test_emergency_client.py](src/mobility_scenarios_src/emergency_priority/tests/test_emergency_client.py)
- [src/mobility_scenarios_src/emergency_priority/tests/test_traffic_light_rules.py](src/mobility_scenarios_src/emergency_priority/tests/test_traffic_light_rules.py)
- [src/mobility_scenarios_src/emergency_priority/tests/test_trafficlight_simulator.py](src/mobility_scenarios_src/emergency_priority/tests/test_trafficlight_simulator.py)
- [src/mobility_scenarios_src/emergency_priority/unified_demo.py](src/mobility_scenarios_src/emergency_priority/unified_demo.py)

### Where the tests live

Yes — these tests should live inside [src/mobility_scenarios_src/emergency_priority/tests](src/mobility_scenarios_src/emergency_priority/tests).

Recommended layout:

- `tests/test_coordinator.py` → unit tests for the priority policy and scenario resolution.
- `tests/test_emergency_client.py` → unit tests with fake/mock dependencies.
- `tests/test_traffic_light_rules.py` → pure unit tests for traffic-light motion mapping.
- `tests/test_trafficlight_simulator.py` → integration tests for the simulator behavior.
- `unified_demo.py` → local smoke/demo entry point, not a unit test.
