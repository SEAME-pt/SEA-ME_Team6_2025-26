#!/usr/bin/env python3
"""
Generate Trustable-format items (EXPECT / ASSERT / EVID) from tsf-requirements-table.md

Usage:
  python3 generate_from_requirements.py --input ../../requirements/tsf-requirements-table.md
...
REQ = ROOT / 'docs' / 'TSF' / 'requirements' / 'tsf-requirements-table.md'
...
def format_references_section(paths):
    if not paths:
        return ""
    s = 'references:\n'
    for p in paths:
...
def make_items(requirements, overwrite=False, run_trudag=False):
    ...
    for idx, (rid, desc) in enumerate(requirements, start=1):
        ...
        refs = format_references_section(evidence_files)
        expect_path = OUT / 'expectations' / f"{uid}.md"
        expect_content = TEMPLATE_EXPECT.format(id=uid, header=header.replace('"','\"'), text=desc, references_section=refs)
