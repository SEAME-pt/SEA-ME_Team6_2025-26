...
This document is the practical junction of the incident-resolution folder:
...
### April 16, 2026 - TruDAG Link-Review Order Fix

**Issue**: Score propagation was inconsistent: many `EXPECTATIONS-*` and `ASSERTIONS-*` stayed at `0.0; Missing` while `EVID`/`ASSUMP` could still reach `1.0`.

**Root Cause**:
1. Link creation was attempted before all items were marked reviewed
2. TruDAG requires reviewed items for valid link creation
3. Reasoning links were partially missing, breaking recursive propagation paths

**Fix Applied**:
- Added pre-link review step in setup pipeline:
	- `docs/TSF/tsf_implementation/scripts/setup_trudag_clean.sh`
- New order enforced: create items -> set reviewed -> create links -> lint/score/publish
- Restored per-item review logs in the new pre-link step for visibility
...
| Incomplete score propagation to EXPECT/ASSERT | Fixed setup order so reviewed status is set before link creation | `TRUDAG_LINK_REVIEW_ORDER_FIX_REPORT.md` |
