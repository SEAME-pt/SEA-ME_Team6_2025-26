...
def main():
    ...
    for core,items in sorted(groups.items()):
        ...
        # ASSERT -> EVID (ASSERT.id -> EVID.id)
        # evidence should be child of the assertion (assert is parent)
        if assert_:
            for e in evids:
                edges.append((assert_, e, 'assert->evid (id core match)', 90))
    ...
    # Canonical output: always write a single `graph.dot` file.
    # If the provided --out ends with .dot use that path, otherwise force `.dot` suffix.
    out_str = str(out_path)
    if out_str.endswith('.dot'):
        graph_dot_path = out_path
    else:
        graph_dot_path = out_path.with_suffix('.dot')

    # write canonical graph.dot only
    graph_dot_path.write_text('\n'.join(dot_lines)+'\n', encoding='utf-8')

    # (No proposed sidecars or CSVs are generated anymore.)
    print('Wrote graph to', graph_dot_path)
