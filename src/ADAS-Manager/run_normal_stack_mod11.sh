...
require_file() {
    local p="$1"
    if [[ ! -e "$p" ]]; then
        echo "[stack:normal-mod11] missing required file: $ROOT/$p"
        exit 2
    fi
}
...
if ! kill -0 "$ADAS_PID" 2>/dev/null; then
    echo "[stack:normal-mod11] adas_manager exited during startup"
    exit 1
fi
...
while true; do
    if [[ "$STOPPING" -eq 1 ]]; then
        echo "[stack:normal-mod11] stop requested"
        exit 0
    fi

    if ! kill -0 "$ADAS_PID" 2>/dev/null; then
        echo "[stack:normal-mod11] adas_manager exited"
        exit 1
    fi
    if ! kill -0 "$JOY_PID" 2>/dev/null; then
        echo "[stack:normal-mod11] joystick_control exited"
        exit 1
    fi
    sleep 1
done