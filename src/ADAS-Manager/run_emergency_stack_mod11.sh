...
require_file() {
    local p="$1"
    if [[ ! -e "$p" ]]; then
        echo "[stack:emergency-mod11] missing required file: $ROOT/$p"
        exit 2
    fi
}
...
require_file "roadside_emergency_controller.py"
...
on_term() {
    echo "[stack:emergency-mod11] stop requested"
    /usr/bin/python3 - <<'PY'
import os
import socket
import time
...
try:
    ...
    time.sleep(0.5)
    for cmd in cmds:
        print(f"[stack:emergency-mod11] safe-state -> {cmd}", flush=True)
        ser.write((cmd + "\r\n").encode("utf-8"))
        ser.flush()
        ...
        while time.time() < deadline:
            ...
            if line:
                print(f"[stack:emergency-mod11] gateway: {line}", flush=True)
    ser.close()
except Exception as exc:
    print(f"[stack:emergency-mod11] safe-state serial error: {exc}", flush=True)
PY
    sleep 3
    STOPPING=1
}
...
if ! kill -0 "$ADAS_PID" 2>/dev/null; then
    echo "[stack:emergency-mod11] adas_manager exited during startup"
    exit 1
fi
...
/usr/bin/python3 roadside_emergency_controller.py \
...
PY
    ...
    if ! kill -0 "$ADAS_PID" 2>/dev/null; then
        echo "[stack:emergency-mod11] adas_manager exited"
        exit 1
    fi
    if ! kill -0 "$JOY_PID" 2>/dev/null; then
        echo "[stack:emergency-mod11] joystick_control exited"
        exit 1
    fi
    if ! kill -0 "$EMERG_PID" 2>/dev/null; then
        echo "[stack:emergency-mod11] roadside_emergency_controller exited"
        exit 1
    fi
    sleep 1
done
