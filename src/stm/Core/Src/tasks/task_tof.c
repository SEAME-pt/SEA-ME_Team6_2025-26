...
{
    sys_log(ctx, "\r\n[ToF] Thread iniciada - 8x8 (Distance only)\r\n");
    ...
    if (s_tof.status) {
        sys_log(ctx, "[ToF] ERRO: Init falhou! status=%u\r\n", s_tof.status);
        s_tof.initialized = 0;
        return;
    }
    ...
    if (s_tof.status) {
        sys_log(ctx, "[ToF] ERRO: Falha ao configurar 8x8! status=%u\r\n", s_tof.status);
        s_tof.initialized = 0;
        return;
    }
    ...
    if (s_tof.status) {
        sys_log(ctx, "[ToF] AVISO: Falha ao configurar frequencia (status=%u)\r\n", s_tof.status);
        /* not fatal */
    }
    ...
    if (s_tof.status) {
        sys_log(ctx, "[ToF] ERRO: Start ranging falhou! status=%u\r\n", s_tof.status);
        s_tof.initialized = 0;
        return;
    }
