#!/bin/bash
# ============================================================
# OTA Device Setup Script - SEA:ME Team 6
# Run this script ON the AGL device to set up automatic OTA
# ============================================================

set -e

echo "=============================================="
echo "  OTA Device Setup - SEA:ME Team 6"
echo "=============================================="
echo ""
...
# -------- Create directories --------
echo "[1/6] Creating OTA directories..."
mkdir -p /opt/ota/logs
mkdir -p /opt/ota/downloads
mkdir -p /opt/ota/releases
mkdir -p /opt/ota/backup

# -------- Create ota-update.sh --------
echo "[2/6] Installing ota-update.sh..."
cat > /opt/ota/ota-update.sh << 'OTAUPDATE'
#!/bin/bash
# ============================================================
...
detect_platform() {
    ...
    if [ "$arch" = "aarch64" ]; then
        ...
        echo "[$(date '+%Y-%m-%d %H:%M:%S')] WARNING: Unknown architecture $arch" >> "$LOG"
        echo "combined"
    fi
}
...
echo "  ✓ ota-update.sh installed"

# -------- Create ota-check.sh --------
echo "[3/6] Installing ota-check.sh..."
cat > /opt/ota/ota-check.sh << 'OTACHECK'
#!/bin/bash
# ============================================================
...
echo "  ✓ ota-check.sh installed"

# -------- Create systemd service --------
echo "[4/6] Installing ota-check.service..."
cat > /etc/systemd/system/ota-check.service << 'SERVICE'
[Unit]
Description=OTA Update Check Service
Documentation=https://github.com/SEAME-pt/SEA-ME_Team6_2025-26
...
SyslogIdentifier=ota-check
User=root
SERVICE

echo "  ✓ ota-check.service installed"

# -------- Create systemd timer --------
echo "[5/6] Installing ota-check.timer..."
cat > /etc/systemd/system/ota-check.timer << 'TIMER'
[Unit]
Description=OTA Update Check Timer - Runs every 15 minutes
Documentation=https://github.com/SEAME-pt/SEA-ME_Team6_2025-26
...
Unit=ota-check.service

[Install]
WantedBy=timers.target
TIMER

echo "  ✓ ota-check.timer installed"

# -------- Enable and start timer --------
echo "[6/6] Enabling OTA services..."
...
echo "  /opt/ota/ota-update.sh    - Update script"
echo "  /opt/ota/ota-check.sh     - Polling script"
echo "  /etc/systemd/system/ota-check.service"
echo "  /etc/systemd/system/ota-check.timer"
echo ""
echo "Timer status:"
systemctl list-timers | grep ota || echo "  (timer not active yet)"
echo ""
echo "To enable automatic updates:"
echo "  echo 'enabled' > /etc/ota-auto-update"
echo ""
echo "To manually trigger update:"
echo "  /opt/ota/ota-update.sh <VERSION>"
echo ""
echo "To check for updates now:"
echo "  /opt/ota/ota-check.sh"
echo ""
echo "To view logs:"
echo "  cat /opt/ota/logs/ota-check.log"
echo "  cat /opt/ota/logs/ota.log"
echo ""
