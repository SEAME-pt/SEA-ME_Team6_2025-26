#!/usr/bin/env python3
"""
SEAME — Dual Inference Pipeline v3
...
Lane: calibração + homografia + TemporalSmoother + socket ADAS Manager.
YOLO: best_model 17 classes, decoder DFL manual, NMS numpy.
"""
...
OUTPUT_PATH   = "/data/demo_dual_v3.mp4"
...
YOLO_STREAMS = [
    ("best_model/conv41", "best_model/conv42", 8),   # 80×80
    ("best_model/conv52", "best_model/conv53", 16),  # 40×40
    ("best_model/conv62", "best_model/conv63", 32),  # 20×20
]
REG_MAX = 16
...
class Calibration:
    def __init__(self, is_rear=False):
        ...
        self.enabled = True
        print(f"[CALIB] [{cam_label}] OK — intrinsic + extrinsic ({calib_dir.name}/) loaded")
...
def calc_lateral_deviation_cm(lanes, calib):
    ...
    if has_left and has_right:
        ...
        return -(lane_center) - CAMERA_OFFSET_CM, "both"
    if has_left:
...
def decode_yolov8_outputs(raw_outputs, conf_thresh=CONF_THRESH, iou_thresh=IOU_THRESH):
    ...
    for cv2_key, cv3_key, stride in YOLO_STREAMS:
        cv2_out    = raw_outputs[cv2_key][0]   # [H, W, 64]
        cv3_out    = raw_outputs[cv3_key][0]   # [H, W, 17]
...
def draw_detections(frame, boxes, scores, classes):
    ...
    for box, score, cls in zip(boxes, scores, classes):
        ...
        cv2.rectangle(frame, (x1, ty - th - baseline - 2), (x1 + tw + 4, ty + 2),
                      (0, 0, 0), -1)
        cv2.putText(frame, label, (x1 + 2, ty - baseline),
...
def draw_overlay_dual(frame, fps, frame_idx,
    ...
    cv2.putText(frame, "SEAME | UFLDv2 + best_model (dual)",
                (w - 250, h - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (200, 200, 200), 1)
    return frame
...
def run(frame_queue, duration_seconds=60, save_video=False, is_rear=False):
    ...
    print(f"Modo:              único VDevice, inferência sequencial")
    ...
    with VDevice() as target:
        ...
        try:
            while (time.time() - t_start) < duration_seconds:
                ...
                loc_row_lanes12, exist_row_lanes12 = postprocess(conv37, W1, b1, W2, b2, keep_rows)
                lanes = decode_lanes(loc_row_lanes12, exist_row_lanes12, single_lane_mode=single_lane_mode)
                ...
                # ── Métricas ───────────────────────────────────────────────
                t_total   = t_lane_pre + t_lane_hailo + t_lane_post + t_coords + t_yolo_hailo + t_yolo_post
...
if __name__ == "__main__":
    ...
    p = argparse.ArgumentParser(description="SEAME Dual Inference — UFLDv2 + best_model")
    ...
    print(f"SEAME | Dual Inference: UFLDv2 (lane) + best_model (objects)")
