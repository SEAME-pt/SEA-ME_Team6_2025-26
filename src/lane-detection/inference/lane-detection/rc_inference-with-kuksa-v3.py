...
LKA_WAYPOINTS_PATH  = "Vehicle.ADAS.LaneKeepAssist.LaneWaypoints"
LKA_POLYNOMIAL_PATH = "Vehicle.ADAS.LaneKeepAssist.LanePolynomial"
...
INTRINSIC_DIR   = Path("/data/seame-configs/camera")  # shared — same lens
...
def _kuksa_worker(q):
    ...
    while True:
        deviation, status, waypoints_json, poly_json = q.get()
        try:
            ...
            req_wp = val_pb2.PublishValueRequest()
            req_wp.signal_id.path = LKA_WAYPOINTS_PATH
            req_wp.data_point.value.string = waypoints_json
            stub.PublishValue(req_wp, metadata=metadata)

            req_poly = val_pb2.PublishValueRequest()
            req_poly.signal_id.path = LKA_POLYNOMIAL_PATH
            req_poly.data_point.value.string = poly_json
            stub.PublishValue(req_poly, metadata=metadata)
        except Exception as e:
            print(f"[KUKSA] Erro: {e}")


def publish_lane_data(deviation, status, waypoints_json, poly_json):
    try:
        _kuksa_queue.put_nowait((
...
# ── Lane trajectory ────────────────────────────────────────────────────────────
def compute_lane_trajectory(lanes, calib):
    """
    Build waypoints and fit a 2nd-degree polynomial from the detected lanes.

    Waypoints are the centre-line in world coordinates (cm): [[x, y], ...],
    sorted by increasing y (near → far).  Returns (waypoints_json, poly_json).
    poly_json is "[a, b, c]" such that x_lateral = a·y² + b·y + c.
    Either string is "" when there is insufficient data.
    """
    left_pts  = lanes[1]   # [(x_px, y_px), ...]
    right_pts = lanes[2]   # [(x_px, y_px), ...]
    ...
    # Build centre-line points in pixel space first, then project to world.
    if has_left and has_right:
        # Match left/right by closest y, average x.
        left_arr  = np.array(sorted(left_pts,  key=lambda p: p[1]), dtype=np.float32)
        right_arr = np.array(sorted(right_pts, key=lambda p: p[1]), dtype=np.float32)
        # Sample at left's y values; interpolate right x for each.
        right_x_interp = np.interp(left_arr[:, 1], right_arr[:, 1], right_arr[:, 0])
        center_px = np.stack(
            [(left_arr[:, 0] + right_x_interp) / 2.0, left_arr[:, 1]], axis=1
        )
    elif has_left:
        center_px = np.array(sorted(left_pts, key=lambda p: p[1]), dtype=np.float32)
    else:
        center_px = np.array(sorted(right_pts, key=lambda p: p[1]), dtype=np.float32)

    # Project to world (cm) if calibration available.
    if calib.enabled:
        pts_world = calib.pixels_to_world(center_px.tolist())
        if pts_world is None:
            return "", ""
        # Sort by distance (y in world = forward distance)
        pts_world = pts_world[np.argsort(pts_world[:, 1])]
        waypoints = pts_world.tolist()
    else:
        # Fall back to normalised pixel coords (no cm units)
        pts_world = center_px[np.argsort(center_px[:, 1])]
        waypoints = pts_world.tolist()

    waypoints_json = json.dumps([[round(p[0], 2), round(p[1], 2)] for p in waypoints])

    # Fit x = a·y² + b·y + c (need ≥ 3 points)
    if len(waypoints) >= 3:
        ys = np.array([p[1] for p in waypoints])
        xs = np.array([p[0] for p in waypoints])
        try:
            coeffs = np.polyfit(ys, xs, 2)
            poly_json = json.dumps([round(float(c), 6) for c in coeffs])
        except np.linalg.LinAlgError:
            poly_json = ""
    else:
        poly_json = ""

    return waypoints_json, poly_json


# ── Pipeline principal ─────────────────────────────────────────────────────────
def run(frame_queue, duration_seconds=60, save_video=False, is_rear=False):
    global CAMERA_OFFSET_CM
    threading.Thread(target=_kuksa_worker, args=(_kuksa_queue,), daemon=True).start()

    # Load rear config overrides if needed
    if is_rear:
        try:
            from config_rear import CAMERA_OFFSET_CM as _rear_offset
            CAMERA_OFFSET_CM = _rear_offset
            print(f"[CONFIG] Rear camera offset: {CAMERA_OFFSET_CM} cm")
        except ImportError:
            print(f"[CONFIG] config_rear.py not found — using front offset ({CAMERA_OFFSET_CM} cm)")
            ...
            with InferVStreams(network_group, input_params, output_params) as pipeline:
                ...
                try:
                    while (time.time() - t_start) < duration_seconds:
                        ...
                        waypoints_json, poly_json = compute_lane_trajectory(lanes, calib)
                        t_coords          = (time.time() - t0) * 1000

                        publish_lane_data(deviation, status, waypoints_json, poly_json)
...
if __name__ == "__main__":
    ...
    p = argparse.ArgumentParser(description="LKA Inference — UFLD + Kuksa")
